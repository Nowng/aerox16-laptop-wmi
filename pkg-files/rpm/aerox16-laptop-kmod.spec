...
# popd
mkdir %{name}-%{version}
mv aerox16-laptop.c %{name}-%{version}
...
for kernel_version in %{?kernel_versions}; do
    ...
    install -D -m 755 _kmod_build_${kernel_version%%___*}/aerox16-laptop.ko  ${RPM_BUILD_ROOT}%{kmodinstdir_prefix}/${kernel_version%%___*}/%{kmodinstdir_postfix}/aerox16-laptop.ko
done
%{?akmod_install}
